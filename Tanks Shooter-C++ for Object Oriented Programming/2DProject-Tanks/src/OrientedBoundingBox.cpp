...
OrientedBoundingBox::OrientedBoundingBox(const sf::Sprite& Object) // Calculate the four points of the OBB from a transformed (scaled, rotated...) RectangleShape
{
	...
	float width = Object.getLocalBounds().size.x;
	float height = Object.getLocalBounds().size.y;
	...
	Points[1] = trans.transformPoint(sf::Vector2f{ width, 0.f });
	Points[2] = trans.transformPoint(sf::Vector2f{ width, height } );
	Points[3] = trans.transformPoint(sf::Vector2f{ 0.f, height });
}
